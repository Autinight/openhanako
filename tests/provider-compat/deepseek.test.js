...
describe("provider-compat/deepseek — apply 主流程接入 reasoning_content 恢复/校验", () => {
  ...
  it("实验开启时只在出站 payload 的第一条 user 消息末尾临时追加 DeepSeek roleplay marker", () => {
    const payload = {
      model: "deepseek-v4-pro",
      messages: [
    ...
    const result = deepseek.apply(payload, deepseekModel, {
      ...
      deepseekRoleplayReasoningContext: {
        locale: "zh-CN",
        agentName: "青岚",
        agentDescription: "沉静细腻的文学型助手",
      },
    });

    expect(result.messages[0].content).toContain("第一条用户消息");
    expect(result.messages[0].content).toContain("〖角色沉浸要求〗");
    expect(result.messages[0].content).toContain("Agent 身份“青岚”");
    expect(result.messages[0].content).toContain("花名册简介：沉静细腻的文学型助手");
    expect(result.messages[0].content).toContain("reasoning_content / thinking");
    expect(result.messages[0].content).not.toContain("Hana DeepSeek roleplay reasoning patch");
    expect(result.messages[2].content).toBe("第二条用户消息");
    expect(payload).toEqual(snapshot);
  });

  it("实验开启且英文 locale 时追加英文 Agent roleplay marker", () => {
    const payload = {
      model: "deepseek-v4-pro",
    ...
    expect(result.messages[0].content).not.toContain("〖角色沉浸要求〗");
  });

  it("实验开启但 thinking off 时不追加 DeepSeek roleplay marker", () => {
    const payload = {
      model: "deepseek-v4-pro",
      messages: [{ role: "user", content: "第一条用户消息" }],
    };
    ...
    expect(result.messages[0].content).toBe("第一条用户消息");
    expect(result.messages[0].content).not.toContain("〖角色沉浸要求〗");
  });

  it("实验关闭时保持第一条 user 消息原样", () => {
    const payload = {
      model: "deepseek-v4-pro",
      messages: [{ role: "user", content: "第一条用户消息" }],
      reasoning_effort: "high",
  ...
  it("实验开启但模型不是 DeepSeek V4 时保持第一条 user 消息原样", () => {
  ...
  it("实验开启时支持数组 content，并只复制被修改的 user message", () => {
    const firstUser = {
      role: "user",
      content: [
        { type: "text", text: "看这张图" },
    ...
    const secondUser = { role: "user", content: "继续" };
    const payload = {
      model: "deepseek-v4-pro",
    ...
    expect(result.messages[0].content[0].text).toBe("看这张图");
    expect(result.messages[0].content[2].text).toContain("〖角色沉浸要求〗");
    expect(firstUser.content[0].text).toBe("看这张图");
  });
